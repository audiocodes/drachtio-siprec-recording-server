...
module.exports = (req, res) => {
  ...
  const opts = {
    ...
    callDetails: {
      ...
      'ac-session-id': req.get('AC-Session-ID'),
      'x-account-id': req.get('X-Account-ID'),
      'x-server-id': req.get('X-Server-ID')
    }
  };
...
function allocateEndpoint(which, rtpEngine, totag, opts) {
  const sdp = which === 'caller' ? opts.sdp1 : opts.sdp2;
  const args = {
    ...opts.callDetails,
    sdp,
    ...
    metadata: `*** ${JSON.stringify(opts.callDetails)}`,
    ...(which === 'callee') ? { 'to-tag': totag } : {}
  };
